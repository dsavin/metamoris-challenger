...
class UserManager implements UserProviderInterface
{
    ...
    public function loadUserByUsername($username)
    {
        if (strpos($username, '@') !== false) {
            ...
            if (!$user) {
                throw new UsernameNotFoundException(sprintf('Email "%s" does not exist.',
                    $username));
            }
    ...
    public function findBy(array $criteria = array(), array $options = array())
    {
        ...
        if (array_key_exists('limit', $options)) {
            list ($offset, $limit) = is_array($options['limit']) ? $options['limit'] : array(
                0,
                $options['limit']
            );
    ...
    protected function getUserCustomFields($userId)
    {
        ...
        $rows = $this->conn->fetchAll('SELECT * FROM ' . $this->conn->quoteIdentifier($this->userCustomFieldsTableName) . ' WHERE user_id = ?',
            array($userId));
        foreach ($rows as $row) {
    ...
    {
        return ($class === 'SimpleUser\User') || is_subclass_of($class,
            'SimpleUser\User');
    }
    ...
    {
        return call_user_func($this->getPasswordStrengthValidator(), $user,
            $password);
    }
    ...
    {
        $this->dispatcher->dispatch(UserEvents::BEFORE_INSERT,
            new UserEvent($user));
        ...
        $this->dispatcher->dispatch(UserEvents::AFTER_INSERT,
            new UserEvent($user));
    }
    ...
    protected function saveUserCustomFields(User $user)
    {
        $this->conn->executeUpdate('DELETE FROM ' . $this->conn->quoteIdentifier($this->userCustomFieldsTableName) . '
            WHERE ' . $this->getUserColumns('user_id') . ' = ?',
            array($user->getId()));
    ...
    {
        $this->dispatcher->dispatch(UserEvents::BEFORE_UPDATE,
            new UserEvent($user));
        ...
        $this->dispatcher->dispatch(UserEvents::AFTER_UPDATE,
            new UserEvent($user));
    }
    ...
    {
        $this->dispatcher->dispatch(UserEvents::BEFORE_DELETE,
            new UserEvent($user));

        $this->clearIdentityMap($user);

        $this->conn->executeUpdate('DELETE FROM ' . $this->conn->quoteIdentifier($this->userTableName) . ' WHERE ' . $this->getUserColumns('id') . ' = ?',
            array($user->getId()));
        $this->conn->executeUpdate('DELETE FROM ' . $this->conn->quoteIdentifier($this->userCustomFieldsTableName) . ' WHERE ' . $this->getUserColumns('user_id') . ' = ?',
            array($user->getId()));

        $this->dispatcher->dispatch(UserEvents::AFTER_DELETE,
            new UserEvent($user));
    }
    ...
    public function clearIdentityMap($user = null)
    {
        if ($user === null) {
            ...
            if ($user instanceof User && array_key_exists($user->getId(),
                    $this->identityMap)
            ) {
                unset($this->identityMap[$user->getId()]);
            } else {
                if (is_numeric($user) && array_key_exists($user,
                        $this->identityMap)
                ) {
